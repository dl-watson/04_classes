...
class Stack {
  ...
  validateBalanced(string) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (val === "(" || val === "[" || val === "{") {
        ...
        // currently debugging the missing case for when the stack is something like : (((( {} ()
        return true;
      } else {
      }
    }
  ...
  evaluate() {
    ...
    const evaluated = this.validateBalanced(string);
...
const string = "func{[[tion[ add(a, b) { return a + b; }";
